...
void handleMans() {  
  ...
  { 
    ...
    if (server.argName(i)=="led")
      mans_led=server.arg(i).toInt();
    if (server.argName(i)=="state")
      mans_state=server.arg(i).toInt();

  }
    pinMode(mans_led, OUTPUT);
    digitalWrite(mans_led, mans_state);
...
void setup(void) {
  ...
  server.on("/mans", handleMans);

  server.on("/LED", handleMansLED);
